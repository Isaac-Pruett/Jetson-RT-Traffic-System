...
public:
    DeepStreamTrackerNode() : Node("deepstream_tracker_node") {
        ...
        this->declare_parameter("video_path", "/home/nvidia/Jetson-RT-Traffic-System/video/clean_vid.mp4");
        ...
        pub_image_  = create_publisher<sensor_msgs::msg::Image>("image_raw", qos_best_effort);
        ...
        watchdog_timer_ = this->create_wall_timer(
            ...
            [this]() {
                if (this->first_frame_received_ && (this->now() - this->last_frame_time_).seconds() > 10.0) {
                    RCLCPP_ERROR(this->get_logger(), "WATCHDOG TRIGGERED: Pipeline silently froze for 10 seconds. Shutting down cleanly.");
                    rclcpp::shutdown();
                }
            }
        );
        ...
        if (source_type_ == 2) RCLCPP_INFO(get_logger(), "Mode: See3CAM (Raw UYVY Input)");
        else RCLCPP_INFO(get_logger(), "Mode: File/Stream (Expanded Buffer Pools Active)");
    }
...
private:
    void start_pipeline() {
        ...
        auto sink_viz   = gst_element_factory_make("fakesink", "sink_viz");

        GstElement *source = nullptr, *demux = nullptr, *parser = nullptr, *decoder = nullptr, *dec_conv = nullptr;
        GstElement *cam_conv = nullptr, *cam_caps = nullptr;
        ...
        if (source_type_ == 0 || source_type_ == 1) { 
            ...
            // CRITICAL FIX: Add 10 extra buffers directly to the hardware decoder pool.
            // This guarantees the decoder never starves if the ROS visualizer runs slowly.
            g_object_set(decoder, "enable-max-performance", 1, "num-extra-surfaces", 10, NULL);
            ...
            source = gst_element_factory_make("v4l2src", "cam-source");
            g_object_set(source, "device", video_path_.c_str(), NULL);
            cam_conv = gst_element_factory_make("nvvideoconvert", "cam_conv");
            g_object_set(cam_conv, "compute-hw", 1, "nvbuf-memory-type", 4, NULL);
            cam_caps = gst_element_factory_make("capsfilter", "cam_caps");
            GstCaps *c = gst_caps_from_string("video/x-raw(memory:NVMM), format=NV12");
            g_object_set(cam_caps, "caps", c, NULL);
            gst_caps_unref(c);
        }

        if (!pipeline_ || !source || !mux || !tracker || !tee || !q_viz) {
            RCLCPP_FATAL(get_logger(), "Failed to create elements!"); return;
        }

        // --- THE ANTI-STARVATION CONFIGURATION ---
        g_object_set(sink_tracker, "sync", TRUE, "qos", FALSE, NULL);
        g_object_set(sink_viz, "sync", FALSE, "qos", FALSE, NULL); 
        
        // CRITICAL FIX: Limit queues to holding exactly 1 frame. 
        // This prevents them from hoarding the NVMM buffer pool.
        g_object_set(q_tracker, "max-size-buffers", 1, "leaky", 0, NULL);
        g_object_set(q_viz, "max-size-buffers", 1, "leaky", 2, NULL);
        ...
        if (source_type_ == 0 || source_type_ == 1) { 
            ...
            gst_bin_add_many(GST_BIN(pipeline_), source, cam_conv, cam_caps, NULL);
            gst_element_link_many(source, cam_conv, cam_caps, NULL);
            
            GstPad *sinkpad = gst_element_request_pad_simple(mux, "sink_0");
            GstPad *srcpad = gst_element_get_static_pad(cam_caps, "src");
            gst_pad_link(srcpad, sinkpad);
            gst_object_unref(srcpad); gst_object_unref(sinkpad);
        }
        ...
        GstPad *viz_pad = gst_element_get_static_pad(sink_viz, "sink");
        gst_pad_add_probe(viz_pad, GST_PAD_PROBE_TYPE_BUFFER, probe_image, this, NULL);
        gst_object_unref(viz_pad);
        ...
        while (GstMessage *msg = gst_bus_pop(bus)) {
            switch (GST_MESSAGE_TYPE(msg)) {
                ...
                case GST_MESSAGE_EOS:
                    RCLCPP_INFO(get_logger(), "EOS Reached. Video completed successfully. Shutting down.");
                    rclcpp::shutdown();
                    break;
                default:
                    break;
    ...
    static GstPadProbeReturn probe_image(GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
        auto *node = (DeepStreamTrackerNode *)user_data;
        if (node->pub_image_->get_subscription_count() == 0) return GST_PAD_PROBE_OK;
        GstBuffer *buf = GST_PAD_PROBE_INFO_BUFFER(info);
        GstMapInfo map;
