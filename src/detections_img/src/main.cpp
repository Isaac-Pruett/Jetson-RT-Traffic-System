...
#include "rclcpp/qos.hpp" // <--- Added for QoS
...
public:
    DrawingNode() : Node("detections_img_node") {
        RCLCPP_INFO(this->get_logger(), "Drawing node started. Listening for /image_raw (Best Effort) and /traffic_detections");

        this->declare_parameter<int>("trail_length", 30);
        max_trail_length_ = this->get_parameter("trail_length").as_int();

        // FIX: Use SensorDataQoS (Best Effort) to match the Streamer
        image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
            "/image_raw", 
            rclcpp::SensorDataQoS(), 
            std::bind(&DrawingNode::image_callback, this, std::placeholders::_1));

        // Boxes can stay reliable/default
    ...
    void try_render() {
        ...
        if (latest_boxes_) {
            for (const auto &box : latest_boxes_->detections) {
                ...
                if (trail.size() > static_cast<size_t>(max_trail_length_)) trail.pop_front();

                // Draw Logic
                cv::Scalar color = cv::Scalar(0, 255, 0); // Default Green
                if (b_id == "car" || b_id == "0") color = cv::Scalar(0, 0, 255); // Red for Cars
                else if (b_id == "person" || b_id == "2") color = cv::Scalar(255, 0, 0); // Blue for People
                ...
                for (size_t i = 1; i < trail.size(); i++) {
                    cv::line(img, trail[i-1], trail[i], color, 2);
                }
            }
        }
